...
def calculate_AUC(model, xTrain, xTest, yTrain, yTest):
    # train the model
    model.fit(xTrain, yTrain)

    # probabilty estimates
    yHatTrainProb = model.predict_proba(xTrain)
    yHatTestProb = model.predict_proba(xTest)

    # training AUC
    fpr, tpr, thresholds = metrics.roc_curve(yTrain, yHatTrainProb[:, 1])
...
def holdout(model, xFeat, y, testSize):
    ...
    # calculate AUC
    trainAuc, testAuc = calculate_AUC(model, xTrain, xTest, yTrain, yTest)
...
def kfold_cv(model, xFeat, y, k):
    ...
    x_split = np.array_split(xFeat, k)
    ...
    for i in range(k):
        ...
        xTrain = [row for chunk in xTrain for row in chunk]
        xTest = x_split[i]
        yTrain = y_split[0:i] + y_split[i+1:]
        yTrain = [row for chunk in yTrain for row in chunk]
        yTest = y_split[i]

        # calculate AUC
        current_trainAuc, current_testAuc = calculate_AUC(model, xTrain, xTest, yTrain, yTest)
        trainAuc += current_trainAuc
        testAuc += current_testAuc

    # average AUC across k folds
    trainAuc /= k
    testAuc /= k

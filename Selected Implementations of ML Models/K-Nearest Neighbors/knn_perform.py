...
def minmax_range(xTrain, xTest):
    ...
    transform_range(xTrain)
    transform_range(xTest)

    return xTrain, xTest


def transform_range(data: np.array):
    '''
    transform a numpy array with min 0 and max 1
    '''
    for i in range(data.shape[1]):
        feature = data[:, i]
        min = np.min(feature)
        max = np.max(feature)
        feature = (feature / (max - min)) - (min / (max - min))
        data[:, i] = feature
...
def add_irr_feature(xTrain, xTest):
    ...
    xTrain = add_irr_norm(xTrain)
    xTest = add_irr_norm(xTest)

    return xTrain, xTest


def add_irr_norm(data: np.array):
    '''
    add two irrelevent columns to the numpy array
    '''
    irr_feature = np.random.normal(0, 1, size=(data.shape[0], 2))

    new_array = np.append(data, irr_feature, axis=1)
